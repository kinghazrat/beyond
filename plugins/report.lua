...
local function run(msg, matches)
 ...
 if matches[1]:lower() == 'setmaster' and is_admin(msg) then
  ...
  if chk then
   ...
   return '_  ایدی مستر جایگزین شد_'
  else 
   ldata[tostring(msg.chat_id_)]["master"] = {id = idmas}
   save_data(_config.moderation.data, ldata)
   return '_  ایدی مستر ذخیره شد_'
   end
 end
  ...
  if master then
   ...
   function id_cb(arg, data)
    ...
    tdcli.sendMessage(msg.chat_id_, 0, 1,  '_  گزارش شما به مدیر گروه ارسال شد._', 1, 'md')
   ...
   return '_  ایدی مستر تعیین نشده است!_'
  end
